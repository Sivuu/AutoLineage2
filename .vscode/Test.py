...
class Memory():
    ...
    def ReadLineageOffsetValueInt32(self, Process, ModuleName, StartAddress, *OffSetList):
...
if __name__ == '__main__':
    ...
    print("--------------------------------------------------")
    mypid = 1844 #PID cua process
    process = OpenProcess(0x10, False, mypid)
    mem = Memory()
    baseEngine = mem.GetBaseAddressModule(mypid, "Engine.dll", 0x01F9E3D8)
    hp = mem.ReadLineageOffsetValueInt32(process, "Engine.dll", baseEngine, int(0x5C)).value
    mp = mem.ReadLineageOffsetValueInt32(process, "Engine.dll", baseEngine, int(0x60)).value
    cp = mem.ReadLineageOffsetValueInt32(process, "Engine.dll", baseEngine, int(0x64)).value
    hppt = dict()
    mppt = dict()
    baseNWindow = mem.GetBaseAddressModule(mypid, "NWindow.DLL", 0x0134E620)
    for i in range(9):
        hppt[i] = mem.ReadLineageOffsetValueInt32(process, "NWindow.DLL", baseNWindow, int(0x20), int(0x3E8), int(0x214), int(0x444 + i*0x4), int(0x34), int(0x21C)).value
        mppt[i] = mem.ReadLineageOffsetValueInt32(process, "NWindow.DLL", baseNWindow, int(0x20), int(0x3E8), int(0x214), int(0x46C + i*0x4), int(0x34), int(0xC4), int(0x21C)).value
